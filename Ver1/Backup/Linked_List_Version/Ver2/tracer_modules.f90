...
MODULE tracersolver_mod
    ...
    ! work pool variables 
	REAL, ALLOCATABLE, PRIVATE :: work_flux(:) , work_rho(:),work_flux2d(:,:,:) , work_rho2d(:,:) 
	...
	CONTAINS
		...
		SUBROUTINE solve(SELF, dt_in, dx_in, dy_in)

			CLASS(MASSFLUXTRACER), INTENT(IN) :: SELF
            REAL, INTENT(IN) :: dt_in, dx_in, dy_in
		...
		SUBROUTINE advect_2d(SELF, tr, cellHead, N_cell, flux, rho)
            ...
            DO i = 1, nx
                ...
                IF(ASSOCIATED(current)) THEN 
						...
						DO l = 1, work_N_cell2d(i+1,j+1)
	        	            ...
	        	            END IF
               				...
               				IF(p .LT. m_frac)THEN
								...
								CALL SELF%cell_transfer_2d(tr,cellHead2d,work_N_cell_temp2d(i+1,j+1),&
					...
					! Outgoing through bottom face
                	IF(SIGN(1.0,work_flux2d(i+1,j,2)) .LT. 0.0) THEN
                		
						m_frac = abs(work_flux2d(i+1,j,2))*dt*dx/m0

                        IF(debug .EQ. 1) PRINT*,'OUTGOING BOTTOM: m0, m_frac=',m0,m_frac 
		...
		SUBROUTINE cell_transfer_2d(SELF, tr, cellHead, N_src, N_des, tr_num ,x_src, x_des, y_src, y_des)
			...
			CLASS(MASSFLUXTRACER), INTENT(IN) :: SELF
			TYPE(tracer_ptr_2d), INTENT(INOUT) :: tr(:),cellHead(:,:)
			INTEGER, INTENT(IN) :: tr_num, N_src, N_des, x_src, x_des, y_src, y_des ! tr_num: tracer number in linked list 
														! x_src,y_src: source cell
														! x_des,y_des : destination cell								
            INTEGER :: i
