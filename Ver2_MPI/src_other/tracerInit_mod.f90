...
FUNCTION fn(i,j,k) RESULT(fx)
    INTEGER, INTENT(IN) :: i,j,k
    INTEGER :: fx
    
    IF(j .GE. ny/2 - 1 .AND. j .LE. 2 + ny/2 .AND. i .EQ. 1+nx/2) THEN
        fx = N/4   
    ELSE
        fx = 0
    END IF
...
SUBROUTINE initialize_tracer_distribution_1d(N_cell_1d, cellHead_1d, cellHead_bndry_out)

	INTEGER, INTENT(INOUT) :: N_cell_1d(xlow:xhi)
	TYPE (node_ptr), INTENT(INOUT) :: cellHead_1d(xlow:xhi), cellHead_bndry_out(:)
	...
	DO i= 1,2
		ALLOCATE(cellHead_bndry_out(i)%p)
	END DO
	
	cellHead_1d(xlow)%p => cellHead_bndry_out(1)%p
	cellHead_1d(xhi)%p => cellHead_bndry_out(2)%p
...
SUBROUTINE initialize_tracer_distribution_2d(N_cell_2d, cellHead_2d, cellHead_bndry_out)

 	INTEGER, INTENT(INOUT) :: N_cell_2d(xlow:xhi,ylow:yhi)
	TYPE (node_ptr), INTENT(INOUT) :: cellHead_2d(xlow:xhi,ylow:yhi), cellHead_bndry_out(:)
    ...
    DO j = ylow,yhi
		cellHead_2d(xhi,j)%p => cellHead_bndry_out(1)%p
		cellHead_2d(xlow,j)%p => cellHead_bndry_out(2)%p	
	END DO

	DO i = xlow,xhi
		cellHead_2d(i,yhi)%p => cellHead_bndry_out(3)%p
		cellHead_2d(i,ylow)%p => cellHead_bndry_out(4)%p	
	END DO
...
SUBROUTINE initialize_tracer_distribution_3d(N_cell_3d, cellHead_3d, cellHead_bndry_out)

 	INTEGER, INTENT(INOUT) :: N_cell_3d(xlow:xhi,ylow:yhi,zlow:zhi)
	TYPE (node_ptr), INTENT(INOUT) :: cellHead_3d(xlow:xhi,ylow:yhi,zlow:zhi), cellHead_bndry_out(:)

    INTEGER:: i, ilow, j, jlow, k, klow, tr_counter, my_start_id, my_end_id
	...
	DO i = xlow+1, xhi-1
		DO j = ylow+1, yhi-1
			DO k = zlow+1, zhi-1
				N_cell_3d(i,j,k) = fn(i,j,k)
				my_particle_num = my_particle_num + N_cell_3d(i,j,k)
			END DO	
		END DO
    END DO
    ...
    DO i = xlow+1, xhi-1
		DO j = ylow+1, yhi-1
			DO k = zlow+1, zhi-1
				...
				ALLOCATE(cellHead_3d(i,j,k)%p)
				cellHead_3d(i,j,k)%p%id = -1
				cellHead_3d(i,j,k)%p%rank = 1
				cellHead_3d(i,j,k)%p%bf = 0
				cellHead_3d(i,j,k)%p%x = i
				cellHead_3d(i,j,k)%p%y = j
				cellHead_3d(i,j,k)%p%z = k
		
				!PRINT*,'Cell #, N = ',i,j, N_cell_2d(i,j)
				IF(N_cell_3d(i,j,k) .GT. 0) THEN            
					CALL tree_init(cellHead_3d(i,j,k)%p,N_cell_3d(i,j,k),tr_counter,i,j,k)    
				END IF
			END DO		
		END DO
    END DO
    ...
    DO k = zlow, zhi 
		DO j = ylow,yhi
			cellHead_3d(xhi,j,k)%p => cellHead_bndry_out(1)%p
			cellHead_3d(xlow,j,k)%p => cellHead_bndry_out(2)%p
		END DO		
	END DO

    DO k = zlow, zhi
		DO i = xlow,xhi
			cellHead_3d(i,yhi,k)%p => cellHead_bndry_out(3)%p
			cellHead_3d(i,ylow,k)%p => cellHead_bndry_out(4)%p
        END DO			
	END DO
	
    DO j = ylow, yhi
		DO i = xlow,xhi
			cellHead_3d(i,j,zhi)%p => cellHead_bndry_out(5)%p
			cellHead_3d(i,j,zlow)%p => cellHead_bndry_out(6)%p
        END DO			
	END DO	
	...
	DO j = yhi , ylow, -1
		DO i = xlow, xhi 
			WRITE(*,FMT='(i4)', ADVANCE = 'NO'), N_cell_3d(i,j,zlow)
		END DO
		PRINT*,''
	END DO
	PRINT*,''
