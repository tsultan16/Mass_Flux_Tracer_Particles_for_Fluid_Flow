...
TYPE :: MASSFLUXTRACER
	...
	PROCEDURE, PASS(SELF), PRIVATE :: pack_MPI_buffer_out_1d
    PROCEDURE, PASS(SELF), PRIVATE :: pack_MPI_buffer_out_2d
...
! Separate subroutine for insertions into boundary cell. Instead of using a tree in a boundary cell, we use 
! a circular linked list (double-sided access) to keep things simpler
...
SUBROUTINE pack_MPI_buffer_out_1d(SELF, buffer, buffer_size, N_cell, cell_head, bndry, max_particles, all_particles_buffered)

    CLASS(MASSFLUXTRACER), INTENT(IN) :: SELF
	REAL, INTENT(INOUT) :: buffer(:)
    INTEGER, INTENT(IN) :: buffer_size, bndry, max_particles
    INTEGER, INTENT(INOUT) :: N_cell(:)
    TYPE (node_ptr), INTENT(INOUT) :: cell_head(:)
	...
	!####################################################################
    i_b = 2
    top => cell_head(i_bndry)%p%node_R

    counter = 1
	
    DO WHILE(counter .LE. MIN(max_particles, n_bndry))

        particle_data(1) = last_tracer%id   
        particle_data(2) = last_tracer%x
        particle_data(3) = last_tracer%y
        particle_data(4) = last_tracer%z
        ...
        temp => top
        top => top%node_R
        cell_head(i_bndry)%p%node_R => top


		! destroy tracer from memory after its data has been copied
        CALL SELF%destroy_tracer(temp)
    ...
    N_cell(i_bndry) = N_cell(i_bndry) - MIN(max_particles, n_bndry)



END SUBROUTINE pack_MPI_buffer_out_1d



SUBROUTINE pack_MPI_buffer_out_2d(SELF, buffer, buffer_size, N_cell, cell_head, bndry, max_particles, all_particles_buffered)
    ...
    INTEGER :: i, j, counter, n_bndry, i_b, i_v, &
               imin, imax, jmin, jmax
    REAL(8) :: particle_data(4)
    LOGICAL :: buffer_full
    TYPE(node), POINTER :: top, temp
   ...
   SELECT CASE(bndry)
    ...
    ! Loop over all boundary cells and remove particles. Loop exits when buffer is full 
	DO i = imin, imax
		DO j = jmin, jmax
						
			top => cell_head(i,j)%p%node_R
			
			DO WHILE(counter .LE. MIN(max_particles, N_cell(i,j)))
			...
			END DO	
   			...
   			IF(counter .GE. max_particles) EXIT
...
END SUBROUTINE pack_MPI_buffer_out_2d
