...
SUBROUTINE initialize_tracer_distribution_1d(N_cell_1d, cellHead_1d)
	...
	INTEGER, INTENT(INOUT) :: N_cell_1d(:)
	TYPE (node_ptr), INTENT(INOUT) :: cellHead_1d(:)
    INTEGER:: i, ilow, tr_counter

    ilow=1

    ! Sample tracer distribution for testing
    N_cell_1d(:)=0
    N_cell_1d(ilow+nx/2) = N/2
    N_cell_1d(ilow+nx/2+1) = N/2
...
SUBROUTINE initialize_tracer_distribution_2d(N_cell_2d, cellHead_2d)
 	...
 	INTEGER, INTENT(INOUT) :: N_cell_2d(:,:)
	TYPE (node_ptr), INTENT(INOUT) :: cellHead_2d(:,:)

    INTEGER:: i, ilow, j, jlow, tr_counter
    ...
    N_cell_2d(ilow+nx/2,jlow+ny/2) = N/4
    N_cell_2d(ilow+nx/2,jlow+1+ny/2) = N/4
    N_cell_2d(ilow+1+nx/2,jlow+ny/2) = N/4
	N_cell_2d(ilow+1+nx/2,jlow+1+ny/2) = N/4
    ...
    DO j = jlow+0, jlow+ny+2*nb-1
        ...
        IF(N_cell_2d(i,j) .GT. 0) THEN            
            CALL tree_init(cellHead_2d(i,j)%p,N_cell_2d(i,j),tr_counter,i-ilow,j-jlow,-1)    
        END IF
	END DO
    END DO
...
SUBROUTINE initialize_tracer_distribution_3d(N_cell_3d, cellHead_3d)

 	INTEGER, INTENT(INOUT) :: N_cell_3d(:,:,:)
	TYPE (node_ptr), INTENT(INOUT) :: cellHead_3d(:,:,:)

    INTEGER:: i, ilow, j, jlow, k, klow, tr_counter
    ...
    ! Sample tracer distribution for testing
    N_cell_3d(:,:,:)=0
    N_cell_3d(ilow+nx/2,jlow+ny/2, klow+nz/2)     = N/4
    N_cell_3d(ilow+nx/2,jlow+1+ny/2, klow+nz/2)   = N/4
    N_cell_3d(ilow+1+nx/2,jlow+ny/2, klow+nz/2)   = N/4
	N_cell_3d(ilow+1+nx/2,jlow+1+ny/2, klow+nz/2) = N/4
    ...
    DO i = ilow+0, ilow+nx+2*nb-1
		DO j = jlow+0, jlow+ny+2*nb-1
			DO k = klow+0, klow+nz+2*nb-1
				...
				PRINT*,'Cell #, N = ',i,j,k, N_cell_3d(i,j,k)
				IF(N_cell_3d(i,j,k) .GT. 0) THEN            
					CALL tree_init(cellHead_3d(i,j,k)%p,N_cell_3d(i,j,k),tr_counter,i-ilow,j-jlow,k-klow)    
				END IF
