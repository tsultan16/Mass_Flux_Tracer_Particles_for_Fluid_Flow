...
SUBROUTINE advect_2d(SELF, cellHead, N_cell, flux, rho)
	...
	!DO j = jlow+ny, jlow+1, -1
	!DO i = ilow+nx ,ilow+1, -1
        ...
        !PRINT*,'##############################'
        !PRINT*,'Cell #',i,j,'N_cell=',Ntr
		!PRINT*,'##############################'
		
		!PRINT*,'Flux x+=',work_flux_2d(i,j,1)
		!PRINT*,'Flux x-=',work_flux_2d(i,j,1)
		!PRINT*,'Flux y+=',work_flux_2d(i,j,2)
		!PRINT*,'Flux y-=',work_flux_2d(i,j-1,2)
        ...
        DO WHILE(face_count .LT. 4 .AND. Ntr .GT. 0)
			...
			! Decimal part of outgoing tracer number is (weighted) randomly set to either 0 or 1
			...
			!PRINT*,'Moving ',Nout,' tracers from cell',i,j,' to ',i+ix,j+iy
			!PRINT*,''
...
SUBROUTINE cell_face_cycle_2d(SELF, Ntr, Nout ,head_src, head_des, mfrac, x_src, x_des, y_src, y_des)
    ...
    !IF( (x_des .EQ. 11 .AND. y_des .EQ. 7) .OR.  & 
	!	(x_src .EQ. 11 .AND. y_src .EQ. 7) ) THEN	
	!PRINT*,'SOURCE TREE:'
    !CALL print_tree(head_src)
	!PRINT*,'DESTINATION TREE:'
    !CALL print_tree(head_des)
	!END IF
    ...
    DO i = 1, Nout
        ...
        !PRINT*,'Moving tracer # ',Nmove,' in the list.'
        ...
        !PRINT*,'Transfer tracer ID = ', transfer_node%id	  
	...
	!DO iy = ny+2*nb , 1, -1
	!	DO ix = 1, nx+2*nb 
	!		WRITE(*,FMT='(i4)', ADVANCE = 'NO'), work_N_cell_2d(ix,iy)
	!	END DO
	!	PRINT*,''
	!END DO
	!PRINT*,''
        ...
        DO WHILE(face_count .LT. 2 .AND. Ntr .GT. 0)
            ...
            CASE(0)
                ...
                IF(SIGN(1.0,work_flux(i)) .GT. 0.0) THEN
					...
					!PRINT*,'Moving tracers across x+ face.'
            ...
            IF(Nout .GT. 0) THEN
	            ...
	            !PRINT*,'Cell #',i, 'Ntr=',Ntr
                !PRINT*,'Nout = ,',Nout,' xsrc, xdes = ',i,i+ix
...
SUBROUTINE cell_face_cycle_1d(SELF, Ntr, Nout, head_src, head_des, mfrac, x_src, x_des)
    ...
    !Print*,'Ntr = ',Ntr
    !PRINT*,'Moving ',Nout,' tracers from cell',x_src,' to ',x_des
